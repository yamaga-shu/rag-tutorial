...
uv run python hourei/mk-rag-db-from-text.py                   # チャンク分割と埋め込み → hourei/hourei.db（密）+ hourei_passages.jsonl（BM25用）
...
```
質問: 私的使用のための複製はどこまで認められますか？

回答:
第30条により、私的使用のための複製は次の範囲で認められます。

1. 基本的な要件（第30条第1項柱書）
 個人的に又は家庭内その他これに準ずる限られた範囲内での使用（＝私的使用）を目的
 とする場合に限り、その使用者自身が複製することができます。

2. 認められない場合（第30条第1項各号）
 - 第一号：公衆の使用に供するために設置された自動複製機器を用いて複製する場合。
 - 第二号：技術的保護手段の回避を伴って複製する場合。
 （中略）

4. 補償金の支払義務（第30条第3項）
 政令で定めるデジタル方式の録音・録画機器等を用いる者は、著作権者に相当な額の
 補償金を支払わなければなりません。
----------------------------------------
参照した条文:
[1] 出典: 著作権法 第三十条 （私的使用のための複製） (https://laws.e-gov.go.jp/law/345AC0000000048)
（私的使用のための複製）第三十条著作権の目的となつている著作物…は、個人的に又は家庭内 …
[2] 出典: 著作権法 第百十九条  (https://laws.e-gov.go.jp/law/345AC0000000048)
利用容易化プログラムの公衆への提供等を行つた者（当該公衆への提供等のために用いられて …
[3] 出典: 著作権法 第三十条 （私的使用のための複製） (https://laws.e-gov.go.jp/law/345AC0000000048)
がされる部分の占める割合…特定侵害複製…を …
[4] 出典: 著作権法 第四十七条の七 （複製権の制限により作成された複製物の譲渡） (https://laws.e-gov.go.jp/law/345AC0000000048)
十五条第一項、第三十七条第三項…第四十 …
```

> e-Gov 法令API v2（`https://laws.e-gov.go.jp/api/2/law_data/{法令ID}?response_format=json`）を利用している。
> 本文は `{tag, attr, children}` の入れ子ツリーで返るため、スクリプト側で Article（条）ごとに走査して条番号、見出し、本文を取り出している。
